...
export function MarketDataPage() {
    ...
    if (formData.maNghe) {
      ...
      if (selectedCareer) {
        if (formData.loai === "Luong") {
          // Khi là "Lương", tiêu đề là tên nghề
          setFormData((prev) => ({ ...prev, tieuDe: selectedCareer.tenNghe }));
        } else {
          // Khi là "Cơ hội", để trống cho người dùng nhập tên công ty
  ...
  }, [formData.maNghe, formData.loai, careers]);
  ...
  const renderFormFields = () => (
    <Box sx={{ display: "flex", flexDirection: "column", gap: 2.2, mt: 1.5 }}>
        ...
        <FormControl fullWidth size="small" sx={inputSx}>
          ...
          <Select
            ...
            onChange={(e) =>
              setFormData({
                ...formData,
                loai: e.target.value as "Luong" | "CoHoi",
              })
            }
          >
      ...
      <TextField
        ...
        size="small"
        label="Tiêu đề"
        placeholder={formData.loai === 'CoHoi' ? "Nhập tên công ty" : "Tên nghề (tự động)"}
    ...
    <Box sx={{ width: "100%", maxWidth: "100%", overflowX: "hidden", boxSizing: "border-box", px: { xs: 1, sm: 2 } }}>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button variant="outlined" onClick={() => { setIsAddDialogOpen(false); setFormData(initialFormState); }} sx={cancelButtonSx}>Hủy</Button>
      ...
      >
        ...
        <DialogContent>{renderFormFields()}</DialogContent>
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button variant="outlined" onClick={() => { setEditingData(null); setFormData(initialFormState); }} sx={cancelButtonSx}>Hủy</Button>
        ...
        <CardContent sx={{ p: { xs: 1.5, sm: 2.5 }, width: "100%", boxSizing: "border-box" }}>
          ...
          <TableContainer
            ...
            sx={{
              border: `1px solid ${borderColor}`,
              borderRadius: "14px",
              overflowX: "auto", // Cho phép cuộn ngang an toàn khi bảng rộng quá màn hình
              width: "100%",
              maxWidth: "100%",
            }}
          >
            <Table sx={{ tableLayout: "fixed", minWidth: 750 }}>
              <TableHead>
                <TableRow sx={{ bgcolor: "#f9fafb" }}>
                  <TableCell sx={thSx} style={{ width: "12%" }}>Mã nghề</TableCell>
                  <TableCell sx={thSx} style={{ width: "15%" }}>Loại</TableCell>
                  <TableCell sx={thSx} style={{ width: "25%" }}>Tiêu đề</TableCell>
                  <TableCell sx={thSx} style={{ width: "23%" }}>Giá trị</TableCell>
                  <TableCell sx={thSx} style={{ width: "15%" }}>Cập nhật</TableCell>
                  ...
                  <TableRow>
                    <TableCell colSpan={6}>
                      <Box sx={{ py: 5, textAlign: "center" }}>
                        <Typography sx={{ color: textMuted }}>Không tìm thấy dữ liệu.</Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
                )}
